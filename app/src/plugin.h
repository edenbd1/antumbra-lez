...
class AntumbraPlugin : public QObject, public IComponent {
    Q_OBJECT
    Q_PLUGIN_METADATA(IID IComponent_IID FILE "metadata.json")
    Q_INTERFACES(IComponent)

public:
    explicit AntumbraPlugin(QObject* parent = nullptr);
    ~AntumbraPlugin() override;

    // Not part of IComponent — Basecamp reads the module name from
    // metadata.json. Kept as a plain accessor for the preview app.
    QString  name() const { return QStringLiteral("antumbra_lez"); }
    QWidget* createWidget(LogosAPI* api) override;
    void     destroyWidget(QWidget* widget) override;

private:
    ChainBridge* m_bridge = nullptr;
};
